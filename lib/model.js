...
export function validateItem(name, item) {
  ...
  switch (name) {
    ...
    case 'policies': {
      const err = need(['insurer', 'policyNumber', 'type']);
      if (err) return err;
      return ['annual', 'single'].includes(item.type) ? null : `invalid type: ${item.type} (annual|single)`;
    }
    case 'agents': return need(['name', 'kind']);
    default: return null;
  }
}
