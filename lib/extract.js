...
export function emailToText(raw) {
  ...
  // Detect a real email header block — but require a From/To/Cc line that
  // actually carries an address. Insurance schedules have bare "FROM:" / "TO:"
  // lines (the coverage period) that would otherwise be mistaken for headers,
  // stripping the real content below them.
  const head = text.slice(0, 2000);
  const looksLikeEmail = /^(from|to|subject|date|mime-version|received|return-path):/im.test(head)
    && /^(from|to|cc|reply-to):[^\n]*@/im.test(head);
...
export function findDates(text) {
  ...
  // "14 Sep 2026", "1st September, 2026", and the hyphenated "01-Jul-2026"
  // that insurance schedules favour — day, month name, year, any of space /
  // hyphen / comma as separators.
